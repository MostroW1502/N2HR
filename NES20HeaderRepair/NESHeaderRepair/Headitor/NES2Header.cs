...
namespace NES20HeaderRepair.NESHeaderRepair
{
    public class NES2Header : INotifyPropertyChanged
    {
        ...
        public string IdentificationString
        {
            get
            {
                string t = $"{(char)header[0]}{(char)header[1]}{(char)header[2]}";
                t += header[3] == 0x1A ? "<EOF>" : ((char)header[3]).ToString();
                return t;
            }
        }
        ...
        public virtual int PRGROMSize
        {
            get
            {
                if (header[9] == 0xF)
                {
                    // needs more experimentation on how to calculate exactly, still unclear to me.
